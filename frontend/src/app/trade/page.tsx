      ...
      <div className="grid min-h-[640px] gap-4 xl:grid-cols-[210px_minmax(420px,1fr)_340px]">
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <Panel className="relative min-h-[390px] overflow-hidden p-0">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--theme-border)] px-4 py-3"><div><strong>{plan.symbol}</strong><span className="ml-2 text-xs text-[var(--text-muted)]">${plan.entry.toFixed(2)} · +1.24%</span></div><div className="flex gap-1" aria-label="Chart timeframe">{["5m", "15m", "1h", "1D"].map((option) => <button key={option} type="button" aria-pressed={timeframe === option} onClick={() => setTimeframe(option)} className={`h-7 px-2 text-xs ${timeframe === option ? "bg-white text-black" : "text-[var(--text-muted)] hover:bg-[var(--surface-card-hover)]"}`}>{option}</button>)}</div></div>
            <TradeChart />
        ...
        <Panel className="p-0">
          ...
          <div className="space-y-4 p-4">
            ...
            <AlertDialog open={reviewOpen} onOpenChange={setReviewOpen}>
              <AlertDialogTrigger render={<button type="button" disabled={!result.policyPassed} className="h-11 w-full bg-sky-300 text-sm font-semibold text-black disabled:cursor-not-allowed disabled:opacity-35" />}>Review paper order</AlertDialogTrigger>
