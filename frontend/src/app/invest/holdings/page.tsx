    ...
    <div className="min-h-full bg-[var(--theme-bg)] px-4 py-5 text-[var(--text-primary)] lg:px-8">
      <div className="mx-auto max-w-[1540px]">
        ...
        <section className="mt-6">
          <div className="hidden overflow-x-auto rounded-lg border border-[var(--theme-border)] md:block">
            {preferences.railMode === "holdings" ? <table className="w-full min-w-[1120px] text-left text-sm">
              <thead className="border-b border-[var(--theme-border)] bg-[var(--surface-card)] text-xs text-[var(--text-muted)]"><tr><th className="px-5 py-4 font-medium">Security</th><th className="px-5 py-4 font-medium">Portfolio</th><th className="px-5 py-4 font-medium">Currency</th><th className="px-5 py-4 text-right font-medium">Quantity</th><th className="px-5 py-4 text-right font-medium">Value</th><th className="px-5 py-4 text-right font-medium">Weight</th><th className="px-5 py-4 text-right font-medium">Today&apos;s return</th><th className="px-5 py-4 font-medium">Thesis</th><th className="px-5 py-4 font-medium">Next review</th></tr></thead>
              <tbody>{visibleHoldings.map(({ record, quote, value }) => { const thesis = thesisByHolding.get(record.holding.id); return <tr key={record.holding.id} className="h-16 border-b border-[var(--theme-border)] last:border-0 hover:bg-white/[0.025]"><td className="px-5"><div className="flex items-center gap-3"><SymbolMark symbol={record.holding.symbol} /><div><Link href={`/invest/positions/${record.holding.symbol.toLowerCase()}`} className="font-semibold hover:text-emerald-400">{record.holding.symbol}</Link><p className="mt-1 text-xs capitalize text-[var(--text-muted)]">{record.holding.asset_type}</p></div></div></td><td className="px-5">{record.portfolio.name}</td><td className="px-5">{quote?.currency || record.holding.cost_currency || preferences.displayCurrency}</td><td className="px-5 text-right tabular-nums">{preferences.privacyMode ? "••••" : record.holding.quantity.toLocaleString("en-US", { maximumFractionDigits: 6 })}</td><td className="px-5 text-right font-semibold tabular-nums">{preferences.privacyMode ? "••••" : formatMoney(value, preferences.displayCurrency)}</td><td className="px-5 text-right tabular-nums">{preferences.privacyMode ? "••••" : `${totalValue ? ((value / totalValue) * 100).toFixed(1) : "0.0"}%`}</td><td className={cn("px-5 text-right font-semibold tabular-nums", (quote?.change ?? 0) >= 0 ? "text-emerald-400" : "text-rose-400")}>{preferences.privacyMode ? "••••" : formatSignedMoney((quote?.change ?? 0) * record.holding.quantity, preferences.displayCurrency)}</td><td className="px-5"><Status value={thesisStatus(thesis)} /></td><td className="px-5 text-[var(--text-muted)]">{thesis?.next_review_at ? formatDate(thesis.next_review_at) : "Not scheduled"}</td></tr>; })}</tbody>
