    ...
    <div className="min-h-full bg-[var(--theme-bg)] px-4 py-4 text-[var(--text-primary)] lg:px-6">
      <div className="mx-auto max-w-[1720px]">
        ...
        <section className="mt-4">
          <div className="hidden overflow-hidden rounded-2xl border border-[var(--theme-border-strong)] bg-[var(--surface-card-strong)] md:block">
          ...
          <div className="space-y-3 md:hidden">{preferences.railMode === "holdings" ? visibleHoldings.map(({ record, quote, value }) => <Link key={record.holding.id} href={`/invest/positions/${record.holding.symbol.toLowerCase()}`} className="block rounded-2xl border border-[var(--theme-border)] bg-[var(--surface-card)] p-4"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><SymbolMark symbol={record.holding.symbol} /><div><p className="font-semibold">{record.holding.symbol}</p><p className="mt-1 text-xs text-[var(--text-muted)]">{record.portfolio.name} · {thesisStatus(thesisByHolding.get(record.holding.id))}</p></div></div><div className="text-right"><p className="font-semibold tabular-nums">{preferences.privacyMode ? "••••" : formatMoney(value, preferences.displayCurrency)}</p><p className={cn("mt-1 text-xs font-semibold", (quote?.change ?? 0) >= 0 ? "text-emerald-400" : "text-rose-400")}>{preferences.privacyMode ? "••••" : formatSignedMoney((quote?.change ?? 0) * record.holding.quantity, preferences.displayCurrency)}</p></div></div></Link>) : watchlistAssets.map((asset) => { const quote = quotes.get(asset.symbol.toUpperCase()); return <Link key={asset.id} href={`/market?symbol=${encodeURIComponent(asset.symbol)}`} className="flex items-center justify-between rounded-2xl border border-[var(--theme-border)] bg-[var(--surface-card)] p-4"><div className="flex items-center gap-3"><SymbolMark symbol={asset.symbol} /><span className="font-semibold">{asset.symbol}</span></div><span className="font-semibold tabular-nums">{preferences.privacyMode ? "••••" : quote ? formatMoney(quote.price, quote.currency || preferences.displayCurrency) : "—"}</span></Link>; })}</div>
          {!loading && preferences.railMode === "holdings" && !visibleHoldings.length && <div className="rounded-2xl border border-dashed border-[var(--theme-border)] py-12 text-center text-sm text-[var(--text-muted)]">No Investment positions match this filter.</div>}
          {!loading && preferences.railMode === "watchlist" && !watchlistAssets.length && <div className="rounded-2xl border border-dashed border-[var(--theme-border)] py-12 text-center text-sm text-[var(--text-muted)]">No saved securities in this scope.</div>}
        </section>

        <section className="mt-6 rounded-2xl border border-[var(--theme-border)] bg-[var(--surface-card)] p-4">
