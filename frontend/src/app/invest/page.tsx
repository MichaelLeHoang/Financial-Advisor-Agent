...
export default function InvestPage() {
  ...
  const {
    ...
    watchlistAssets, events, recurringBuys, policyValidation, preferences, loading, quotesLoading,
      ...
      <div className="mx-auto max-w-[1840px]">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" title="Refresh investment data" aria-label="Refresh investment data" onClick={() => void refresh()} disabled={loading} className="inline-flex size-10 items-center justify-center rounded-full border border-[var(--theme-border-strong)] hover:bg-[var(--surface-card-hover)] disabled:opacity-45"><RefreshCw className={cn("size-4", loading && "animate-spin")} /></button>
  ...
  return (
    <section className="rounded-lg border border-[var(--theme-border)] bg-[var(--surface-card)] p-5">
      <div className="flex items-center justify-between gap-2">
        <div role="tablist" aria-label="Investment list" className="flex min-w-0 items-center">
          {(["holdings", "watchlist"] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={mode === item} onClick={() => onMode(item)} className={cn("h-10 rounded-full px-2.5 text-sm font-semibold capitalize", mode === item ? "bg-white/[0.10] text-[var(--text-primary)]" : "text-[var(--text-muted)] hover:bg-white/[0.05]")}>{item}</button>)}
        </div>
      ...
      <Link href={mode === "holdings" ? "/invest/holdings" : "/watchlist"} className="mt-4 inline-flex h-10 items-center gap-1 rounded-full px-3 text-sm font-semibold text-emerald-400 hover:bg-emerald-400/10">View all {mode} <ArrowRight className="size-4" /></Link>
    </section>
  );
}
