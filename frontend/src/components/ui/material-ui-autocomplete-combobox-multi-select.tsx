...
export function Autocomplete({
  ...
  const filteredItems = React.useMemo(() => {
    if (filterFn === false || filterFn === null) return items;
    if (filterFn) return items.filter((item) => filterFn(item, debouncedQuery));
  ...
  React.useEffect(() => {
    if (renderState === "open") setLastFilteredItems(filteredItems);
  ...
  const updatePlacementAndHeight = React.useCallback(() => {
    ...
    setMaxHeight(Math.max(100, Math.min(260, availableSpace - 16)));
  }, []);
  ...
  const updateChipScrollFade = React.useCallback(() => {
    ...
    if (leftFadeRef.current) leftFadeRef.current.style.opacity = scrollLeft > 1 ? "1" : "0";
  ...
  const handleSelect = (selectedValue: string) => {
    if (isMulti) {
      ...
      if (isSelected) {
        ...
        onValueChange?.(currentValues.filter((v) => v !== selectedValue));
      } else {
      ...
      case "Enter":
        ...
        if (activeIndex >= 0) {
          if (activeIndex < filteredItems.length) {
            ...
            if (!isSelectDisabled) handleSelect(targetItem.value);
  ...
  const currentIconState = inputValue.length > 0 ? "x" : isOpen ? "up" : "down";
  ...
  const isOptionDisabled = React.useCallback((itemValue: string) => {
    ...
    return !isSelected && typeof maxOptions === "number" && currentValues.length >= maxOptions;
        ...
        <div className="relative flex items-center w-full h-11 flex-shrink-0 pl-4 pr-1.5 z-30">
          <input
            ...
            onChange={(e) => {
              ...
              if (!isOpen) openList(nextActiveIndex);
              else setActiveIndex(nextActiveIndex);
              interactionType.current = "keyboard";
            }}
            onFocus={() => openList()}
                        ...
                        {itemsToRender.map((item, index) => {
                          ...
                          return (
                            <M3AutocompleteItem
                              ...
                              className={cn(isDisabled && "opacity-40 cursor-not-allowed pointer-events-none")}
                            >
                              {isSelected && (
