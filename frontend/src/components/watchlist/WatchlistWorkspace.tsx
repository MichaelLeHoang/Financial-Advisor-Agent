              ...
              <table className="w-full min-w-[860px] text-left text-sm">
                ...
                <tbody>
                  {loading ? <tr><td colSpan={8} className="h-40 text-center text-[var(--text-muted)]"><Loader2 className="mr-2 inline size-4 animate-spin" />Loading live quotes</td></tr> : rows.length ? rows.map((row) => {
                    ...
                    return <tr key={row.id} className="border-t border-[var(--theme-border)] hover:bg-[var(--surface-card-hover)]"><td className="px-4 py-3 font-medium"><Link href={`/discover/markets/stocks/${encodeURIComponent(row.symbol)}`} className="hover:text-indigo-300">{quote?.name || row.symbol}</Link></td><td className="px-4 py-3 text-[var(--text-muted)]">{row.symbol}</td><td className="px-4 py-3 tabular-nums">{quote ? money.format(quote.price) : "—"}</td><td className={cn("px-4 py-3 tabular-nums", (quote?.change ?? 0) >= 0 ? "text-emerald-400" : "text-rose-400")}>{quote ? `${quote.change >= 0 ? "+" : ""}${quote.change.toFixed(2)}%` : "—"}</td><td className="px-4 py-3 tabular-nums">{quote?.market_cap ? `$${compact.format(quote.market_cap)}` : "—"}</td><td className="px-4 py-3 tabular-nums">{quote?.pe_ratio ? quote.pe_ratio.toFixed(1) : "—"}</td><td className="px-4 py-3"><Button size="sm" variant="outline" className="rounded-lg" onClick={() => setAlertTarget({ symbol: row.symbol, alert })}><Bell className="size-3.5" />{alert ? "Edit" : "Add"}</Button></td><td className="px-4 py-3"><Button size="icon" variant="ghost" aria-label={`Remove ${row.symbol}`} className="rounded-lg text-[var(--text-muted)] hover:text-rose-300" onClick={() => void removeSymbol(row)}><Trash2 className="size-4" /></Button></td></tr>;
