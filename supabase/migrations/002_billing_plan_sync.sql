...
alter type public.plan_type add value if not exists 'trader';
alter type public.plan_type add value if not exists 'quant';
alter type public.plan_type add value if not exists 'execution_addon';
...
begin
    ...
    select new.id, 'free'::public.plan_type, 'inactive'
    where not exists (
...
drop policy if exists "Users can manage holdings in their own portfolios" on public.holdings;
...
drop policy if exists "Users can manage assets in their own watchlists" on public.watchlist_assets;
...
drop policy if exists "Service role can update profiles for billing" on public.profiles;
