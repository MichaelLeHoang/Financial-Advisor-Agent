...
from src.core.redis_client import (
    RedisUnavailable,
    get_redis_client,
    normalize_redis_error,
)
from src.agent.activity import planned_steps_for
...
class LLMJobQueue:
    ...
    def enqueue(self, payload: dict[str, Any], kind: JobKind) -> dict[str, Any]:
        ...
        if kind != "memory":
            planned_steps = payload.get("activity_plan") or [
                step.model_dump() for step in planned_steps_for(kind)
            ]
    ...
    def append_activity(
        ...
        """Append one ordered, bounded activity event to a job record."""
        record = self.get(job_id)
        if record is None:
            return None
        events = list(record.get("activity_events") or [])
        next_sequence = int(events[-1].get("sequence") or 0) + 1 if events else 1
        normalized = {
            **event,
            "run_id": job_id,
            "sequence": next_sequence,
            "occurred_at": event.get("occurred_at") or time.time(),
        }
        record["activity_events"] = [*events, normalized][-200:]
        record["updated_at"] = time.time()
        self._write_job(job_id, record)
        return normalized
    ...
    def update_progress(
        ...
        *,
        ...
        record["progress_events"] = [*(record.get("progress_events") or []), progress][
            -80:
        ]
        record["updated_at"] = time.time()
        self._write_job(job_id, record)
    ...
    def dequeue(self, timeout_seconds: int | None = None) -> QueuedJob | None:
        timeout = (
            settings.llm_worker_poll_timeout_seconds
    ...
    def try_acquire_slots(self, job: QueuedJob) -> bool:
        ...
        global_limit = (
            settings.llm_consensus_concurrency
            if job.kind == "consensus"
            else settings.llm_single_concurrency
        )
